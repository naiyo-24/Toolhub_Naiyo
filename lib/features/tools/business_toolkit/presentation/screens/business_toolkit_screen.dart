import 'package:flutter/material.dart';
import 'package:go_router/go_router.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:tool_hub/features/auth/presentation/providers/auth_provider.dart';
import 'package:tool_hub/core/theme/app_colors.dart';
import 'package:tool_hub/core/theme/app_text_styles.dart';
import 'package:tool_hub/core/widgets/neo_card.dart';
import 'package:tool_hub/core/widgets/universal_tool_card.dart';
import 'package:tool_hub/core/utils/snackbar_utils.dart';
import 'package:tool_hub/core/ads/banner_ad_widget.dart';

class BusinessToolkitScreen extends ConsumerStatefulWidget {
  const BusinessToolkitScreen({super.key});

  @override
  ConsumerState<BusinessToolkitScreen> createState() =>
      _BusinessToolkitScreenState();
}

class _BusinessToolkitScreenState extends ConsumerState<BusinessToolkitScreen> {
  String _searchQuery = '';
  bool _showAllTools = false;

  final List<Map<String, dynamic>> tools = [
    {
      'title': 'Inventory Manager',
      'subtitle': 'Track your inventory',
      'icon': Icons.inventory_2_rounded,
      'color': AppColors.primaryBlue,
      'actionText': 'Manage'
    },
    {
      'title': 'Purchase Invoice',
      'subtitle': 'Record purchases',
      'icon': Icons.shopping_cart_checkout_rounded,
      'color': AppColors.primaryRed,
      'actionText': 'Record'
    },
    {
      'title': 'Sales Tracker',
      'subtitle': 'Track daily sales',
      'icon': Icons.point_of_sale_rounded,
      'color': AppColors.primaryGreen,
      'actionText': 'Track'
    },
    {
      'title': 'POS Billing',
      'subtitle': 'Rapid checkout',
      'icon': Icons.storefront_rounded,
      'color': AppColors.primaryRed,
      'actionText': 'Checkout'
    },
    {
      'title': 'Expense Manager',
      'subtitle': 'Manage business expenses',
      'icon': Icons.account_balance_wallet_rounded,
      'color': AppColors.primaryRed,
      'actionText': 'Manage'
    },
    {
      'title': 'Profit Calculator',
      'subtitle': 'Calculate margins',
      'icon': Icons.monetization_on_rounded,
      'color': AppColors.primaryYellow,
      'actionText': 'Calculate'
    },
    {
      'title': 'Invoice Generator',
      'subtitle': 'Create invoices',
      'icon': Icons.receipt_long_rounded,
      'color': AppColors.primaryPurple,
      'actionText': 'Generate'
    },
    {
      'title': 'Business Card',
      'subtitle': 'Generate digital cards',
      'icon': Icons.contact_mail_rounded,
      'color': AppColors.primaryPink,
      'actionText': 'Generate'
    },
    {
      'title': 'Quotation Gen',
      'subtitle': 'Create quotes',
      'icon': Icons.request_quote_rounded,
      'color': AppColors.primaryBlue,
      'actionText': 'Generate'
    },
    {
      'title': 'Receipt Gen',
      'subtitle': 'Create receipts',
      'icon': Icons.receipt_long_outlined,
      'color': AppColors.primaryGreen,
      'actionText': 'Generate'
    },
    {
      'title': 'Business Analytics',
      'subtitle': 'View insights',
      'icon': Icons.analytics_rounded,
      'color': AppColors.primaryYellow,
      'actionText': 'View'
    },
  ];

  void _showComingSoon(BuildContext context) {
    SnackbarUtils.showNeoSnackBar(context,
        message: 'This business tool is coming soon!');
  }

  @override
  Widget build(BuildContext context) {
    var filteredTools = tools.where((tool) {
      final title = (tool['title'] as String).toLowerCase();
      final query = _searchQuery.toLowerCase();
      return title.contains(query);
    }).toList();

    if (_searchQuery.isEmpty && !_showAllTools) {
      filteredTools = filteredTools.take(6).toList();
    }

    return Scaffold(
      backgroundColor: Theme.of(context).scaffoldBackgroundColor,
      body: SafeArea(
        child: Column(
          children: [
            _buildHeader(context),
            _buildSearchBar(),
            Expanded(
              child: SingleChildScrollView(
                child: Column(
                  children: [
                    GridView.builder(
                      shrinkWrap: true,
                      physics: const NeverScrollableScrollPhysics(),
                      padding: const EdgeInsets.all(20),
                      gridDelegate:
                          const SliverGridDelegateWithFixedCrossAxisCount(
                        crossAxisCount: 2,
                        crossAxisSpacing: 16,
                        mainAxisSpacing: 16,
                        childAspectRatio: 0.9,
                      ),
                      itemCount: filteredTools.length,
                      itemBuilder: (context, index) {
                        final tool = filteredTools[index];
                        return _buildUtilityCard(context, tool);
                      },
                    ),
                    if (_searchQuery.isEmpty)
                      Padding(
                        padding: const EdgeInsets.only(bottom: 40),
                        child: GestureDetector(
                          onTap: () {
                            setState(() {
                              _showAllTools = !_showAllTools;
                            });
                          },
                          child: Container(
                            padding: const EdgeInsets.symmetric(
                                horizontal: 32, vertical: 12),
                            decoration: BoxDecoration(
                              color: AppColors.primaryYellow,
                              borderRadius: BorderRadius.circular(12),
                              border: Border.all(color: Colors.black, width: 2),
                              boxShadow: const [
                                BoxShadow(
                                  color: Colors.black,
                                  offset: Offset(4, 4),
                                ),
                              ],
                            ),
                            child: Text(
                              _showAllTools ? 'View Less' : 'View More',
                              style: AppTextStyles.sectionTitle
                                  .copyWith(fontSize: 16),
                            ),
                          ),
                        ),
                      ),
                  
                    const SizedBox(height: 20),
                    const Padding(
                      padding: EdgeInsets.symmetric(horizontal: 20),
                      child: BannerAdWidget(),
                    ),
                    const SizedBox(height: 40),
                  ],
                ),
              ),
            ),
          ],
        ),
      ),
    );
  }

  Widget _buildHeader(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.fromLTRB(20, 16, 20, 16),
      child: NeoCard(
        backgroundColor: const Color(0xFF8B00FF),
        padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 12),
        borderRadius: 12,
        shadowOffset: const Offset(4, 4),
        child: Row(
          children: [
            GestureDetector(
              onTap: () => context.pop(),
              child: Container(
                padding: const EdgeInsets.all(8),
                decoration: BoxDecoration(
                  color: Colors.white,
                  borderRadius: BorderRadius.circular(8),
                  border: Border.all(color: Colors.black, width: 2),
                  boxShadow: const [
                    BoxShadow(color: Colors.black, offset: Offset(2, 2)),
                  ],
                ),
                child: const Icon(Icons.arrow_back_rounded,
                    color: Colors.black, size: 18),
              ),
            ),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.center,
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(
                        'Business ',
                        style: AppTextStyles.heroTitle
                            .copyWith(fontSize: 22, color: Colors.white),
                      ),
                      Text(
                        'Toolkit',
                        style: AppTextStyles.logoText.copyWith(
                            fontSize: 22,
                            color: Colors.white,
                            fontWeight: FontWeight.normal,
                            fontStyle: FontStyle.italic),
                      ),
                    ],
                  ),
                  Text(
                    'MANAGE & GROW',
                    style: AppTextStyles.caption.copyWith(
                        fontSize: 9, letterSpacing: 2, color: Colors.white70),
                  ),
                ],
              ),
            ),
            Row(
              children: [
                GestureDetector(
                  onTap: () => context.push('/edit-profile'),
                  child: Container(
                    padding: const EdgeInsets.all(8),
                    decoration: BoxDecoration(
                      color: Colors.white,
                      borderRadius: BorderRadius.circular(8),
                      border: Border.all(color: Colors.black, width: 2),
                      boxShadow: const [
                        BoxShadow(color: Colors.black, offset: Offset(2, 2)),
                      ],
                    ),
                    child: const Icon(Icons.person_rounded,
                        color: Colors.black, size: 18),
                  ),
                ),
                const SizedBox(width: 8),
                GestureDetector(
                  onTap: () async {
                    await ref.read(authProvider.notifier).signOut();
                    if (context.mounted) {
                      context.pushReplacement('/business-login');
                    }
                  },
                  child: Container(
                    padding: const EdgeInsets.all(8),
                    decoration: BoxDecoration(
                      color: Colors.white,
                      borderRadius: BorderRadius.circular(8),
                      border: Border.all(color: Colors.black, width: 2),
                      boxShadow: const [
                        BoxShadow(color: Colors.black, offset: Offset(2, 2)),
                      ],
                    ),
                    child: const Icon(Icons.logout_rounded,
                        color: Colors.red, size: 18),
                  ),
                ),
              ],
            ),
          ],
        ),
      ),
    );
  }

  Widget _buildSearchBar() {
    return Padding(
      padding: const EdgeInsets.fromLTRB(20, 0, 20, 16),
      child: Container(
        decoration: BoxDecoration(
          color: Theme.of(context).colorScheme.surface,
          borderRadius: BorderRadius.circular(12),
          border: Border.all(
              color: Theme.of(context).colorScheme.onSurface, width: 2),
          boxShadow: [
            BoxShadow(
              color: Theme.of(context).colorScheme.onSurface,
              offset: const Offset(4, 4),
            ),
          ],
        ),
        child: TextField(
          onChanged: (value) {
            setState(() {
              _searchQuery = value;
            });
          },
          decoration: InputDecoration(
            hintText: 'Search business tools...',
            hintStyle: TextStyle(
                color: Theme.of(context)
                    .colorScheme
                    .onSurface
                    .withValues(alpha: 0.5)),
            prefixIcon: Icon(Icons.search_rounded,
                color: Theme.of(context).colorScheme.onSurface),
            border: InputBorder.none,
            contentPadding:
                const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
          ),
          style: TextStyle(color: Theme.of(context).colorScheme.onSurface),
        ),
      ),
    );
  }

  Widget _buildUtilityCard(BuildContext context, Map<String, dynamic> tool) {
    final cardColor = tool['color'] as Color;

    return UniversalToolCard(
      title: tool['title'] as String,
      subtitle: tool['subtitle'] as String?,
      color: cardColor,
      icon: tool['icon'] as IconData,
      actionText: tool['actionText'] as String,
      onTap: () {
        if (tool['title'] == 'Business Analytics') {
          context.push('/business-analytics');
        } else if (tool['title'] == 'Profit Calculator') {
          context.push('/profit-calculator');
        } else if (tool['title'] == 'Invoice Generator') {
          context.push('/invoice-generator');
        } else if (tool['title'] == 'Quotation Gen') {
          context.push('/quotation-gen');
        } else if (tool['title'] == 'Receipt Gen') {
          context.push('/receipt-gen');
        } else if (tool['title'] == 'Business Card') {
          context.push('/business-card');
        } else if (tool['title'] == 'Inventory Manager') {
          context.push('/inventory-list');
        } else if (tool['title'] == 'Sales Tracker') {
          context.push('/sales-tracker');
        } else if (tool['title'] == 'POS Billing') {
          context.push('/pos-billing');
        } else if (tool['title'] == 'Expense Manager') {
          context.push('/business-expense-manager');
        } else if (tool['title'] == 'Purchase Invoice') {
          context.push('/purchase-invoice');
        } else {
          _showComingSoon(context);
        }
      },
    );
  }
}
